...
struct GeneralDownloader {
    url: String,
    name: String,
    selected: Option<GeneralSelection>,
}
...
impl GeneralDownloader {
    ...
    async fn extract_info(&self, url: &str) -> Result<Option<Value>, Report<AppError>> {
        ...
        serde_json::from_str(stdout)
            .map(Some)
            .change_context(AppError::Custom("解析通用下载信息失败".to_string()))
    }
    ...
    fn entry_allowed(&self, entry: &Value) -> bool {
        ...
        Self::string_field(entry, &["webpage_url", "original_url", "url"]).is_some()
    ...
    fn ytdlp_runtime(&self, downloader_type: DownloaderType) -> DownloaderRuntime {
        let selection = self.selected.as_ref().cloned().unwrap_or(GeneralSelection {
...
#[async_trait]
impl DownloadBase for GeneralDownloader {
    async fn check_stream(&mut self) -> Result<StreamStatus, Report<AppError>> {
        ...
        Ok(StreamStatus::Live {
            stream_info: Box::new(StreamInfoExt {
                ...
                raw_stream_url: selection.raw_stream_url,
