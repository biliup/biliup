...
impl Twitch {
    pub fn new(fname: String, url: String, suffix: Option<String>) -> Self {
        Self {
            ...
            proc: None,
            danmaku: None,
            re: Regex::new(r"https?://(?:(?:www|go|m)\.)?twitch\.tv/(?P<id>[0-9_a-zA-Z]+)").unwrap(),
        }
    }

    pub async fn acheck_stream(&mut self, is_check: bool) -> AppResult<bool> {
        let channel_name = self.re
            .captures(&self.url)
        ...
        if self.downloader == "streamlink" || self.downloader == "ffmpeg" {
            // 获取可用端口
            let listener = TcpListener::bind("127.0.0.1:0").await.change_context(AppError::Unknown)?;
            let port = listener.local_addr().change_context(AppError::Unknown)?.port().to_string();
            drop(listener);
            ...
            if let Some(auth_token) = TwitchUtils::get_auth_token() {
                extra_args.push(format!("--twitch-api-header=Authorization=OAuth {}", auth_token));
            }
            ...
            let token = stream.playback_access_token
            ...
            let query_string = serde_urlencoded::to_string(&query_params).change_context(AppError::Unknown)?;
...
impl TwitchUtils {
    ...
    pub async fn post_gql<T: Serialize>(ops: T) -> AppResult<GqlResponse> {
        ...
        for retry in 0..2 {
            ...
            headers.insert("Content-Type", "text/plain;charset=UTF-8".parse().change_context(AppError::Unknown)?);
            headers.insert("Client-ID", CLIENT_ID.parse().change_context(AppError::Unknown)?);

            // 第二次重试时不使用 auth_token（因为已经失效）
            if retry == 0 {
                if let Some(auth_token) = Self::get_auth_token() {
                    headers.insert("Authorization", format!("OAuth {}", auth_token).parse().change_context(AppError::Unknown)?);
                }
            }
            ...
            let resp = client
                ...
                .send()
                .await.change_context(AppError::Unknown)?;

            resp.error_for_status_ref().change_context(AppError::Unknown)?;
