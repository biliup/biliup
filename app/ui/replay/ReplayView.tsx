...
export default function ReplayView({
  ...
  const back = () => {
    ...
    else router.push(historyHref('sessions'))
  }
  const header = (
    <PageHeader
        ...
        <div className={styles.center}>
          {detailLoading ? (
            ...
            <Empty
              ...
              description="这一场可能在升级前录制（没有时间轴），还没写出第一个分段，或者已经被删除。升级前的录像请在「历史记录」的「录制文件」里按文件播放"
            >
              <Button onClick={() => router.push(historyHref('files'))}>去录制文件</Button>
            </Empty>
          ) : (
