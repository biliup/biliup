...
class Douyu(DownloadBase):
    ...
    def check_stream(self, is_check=False):
        ...
        try:
            ...
            video_loop = int(match1(html, r'"videoLoop"\s*:\s*(\d+)'))
            self.room_title = match1(html, r'<h3 class="Title-header">(.*?)</h3>')
            if not room_id:
